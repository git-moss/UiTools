...
public final class Functions
{
     ...
     * @throws IOException Could not read the icon
     */
    public static Image iconFor (final String iconName) throws IOException
    {
        try (final InputStream in = ClassLoader.getSystemResourceAsStream (iconName))
        {
            return iconName == null ? null : new Image (in);
        }
    }
     ...
     * @throws IOException Could not read the icon
     */
    public static Image iconFor (final String iconName, final int size) throws IOException
    {
        try (final InputStream in = ClassLoader.getSystemResourceAsStream (iconName))
        {
            return iconName == null ? null : new Image (in, size, size, false, true);
        }
    }


    /**
     * Get an UTF-8 text file from a jar file.
     *
     * @param fileName The name (and path) of the file
     * @return The text content of the retrieved file
     * @throws IOException Could not extract the file
     */
    public static String textFileFor (final String fileName) throws IOException
    {
        final InputStream systemResourceAsStream = ClassLoader.getSystemResourceAsStream (fileName);
        if (systemResourceAsStream == null)
            throw new IOException ("Resource '" + fileName + "' not found.");
        try (final InputStream in = new BufferedInputStream (systemResourceAsStream))
        {
    ...
    /**
     * Get a binary file from a jar file.
     *
     * @param fileName The name (and path) of the file
     * @return The binary content of the retrieved file
     * @throws IOException Could not extract the file
     */
    public static byte [] rawFileFor (final String fileName) throws IOException
    {
        final InputStream systemResourceAsStream = ClassLoader.getSystemResourceAsStream (fileName);
        if (systemResourceAsStream == null)
            throw new IOException ("Resource '" + fileName + "' not found.");
     ...
     */
    public static int getSelectedToggleIndex (final ToggleGroup toggleGroup)
    {
     ...
     *            toggles
     */
    public static void setSelectedToggleIndex (final ToggleGroup toggleGroup, final int index)
    {
        final ObservableList<Toggle> toggles = toggleGroup.getToggles ();
